...
| # | 条件 | 当前状态 |
|---|------|----------|
| G1 | 独立审查报告 `独立投入使用审查报告.md` 判定从 FAIL 转为 CONDITIONAL 或 PASS | FAIL（见审查报告 P0-1～P0-5） |
| G2 | fake 长任务/同路径/写入收敛验收全部通过 | 未完（P0-1 未闭合） |
| G3 | 用户明确口头或书面授权："授权执行真实 disposable Provider A/B" | 未获得 |
| G4 | disposable 仓库已创建，不含真实项目凭据或敏感数据 | 未创建 |
| G5 | `npm run check` 连续三次全绿（构建+测试退出码 0） | 退出码 1（见审查报告） |
