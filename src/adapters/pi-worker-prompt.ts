...
export function buildPiWorkerPrompt(input: { taskSpec: TaskSpec }): string {
  ...
  return [
    ...
    '6. commitHash 必须等于你在 worktree 中实际提交的 commit 的完整 hash',
    '   （即 `git rev-parse HEAD` 的输出值）。status=completed 时 commitHash 必须是非空真实 hash，',
    '   禁止使用占位符、示例值或随机串；系统会用 worktree HEAD 校验该字段。',
