...
import type { AtomicApplyInput, AtomicApplyResult, AcquirePathLocksInput, AcquirePathLocksResult } from './state-store.js';
...
/**
 * Terminal run statuses that cannot be overwritten.
 */
const TERMINAL_RUN_STATUSES = new Set(['completed', 'failed', 'canceled']);

/**
 * Terminal task statuses that cannot be overwritten.
 */
...
export class SqliteStateStore implements StateStore {
   ...
   */
  static create(dbPath: string, privacyService?: PrivacyService | null): SqliteStateStore {
   ...
   */
  async updateRunStatus(runId: string, status: string, updatedAt: string): Promise<boolean> {
    // Check if run exists and is in a truly terminal state
    const existing = await this.getRun(runId);
    if (!existing) return false;
    // 'failed' and 'canceled' are immutable terminal states.
    if (existing.status === 'failed' || existing.status === 'canceled') {
      return false;
    }
    // 'completed' is terminal for normal paths, but can be overwritten by 'failed'
    // (convergence failure after premature completion).
    if (existing.status === 'completed' && status !== 'failed') {
      return false;
    }

    const stmt = this.db.prepare('UPDATE runs SET status = ?, updated_at = ? WHERE id = ?');
    stmt.run(status, updatedAt, runId);
    return true;
  }
  ...
  async updateTaskStatus(taskId: string, status: string, updatedAt: string): Promise<boolean> {
    const existing = await this.getTask(taskId);
    if (!existing) return false;
    const stmt = this.db.prepare('UPDATE tasks SET status = ?, updated_at = ? WHERE id = ?');
    stmt.run(status, updatedAt, taskId);
    return true;
  }
  ...
  async updateStageStatus(stageId: string, status: string, updatedAt: string): Promise<boolean> {
    const existing = await this.getStage(stageId);
    if (!existing) return false;
    const stmt = this.db.prepare('UPDATE stages SET status = ?, updated_at = ? WHERE id = ?');
    stmt.run(status, updatedAt, stageId);
    return true;
  }
  ...
  async createAttempt(input: CreateAttemptInput): Promise<AttemptRecord> {
    ...
    return {
      ...
      logPath: null, rawLogPath: null, createdAt: now, updatedAt: now,
    };
  }
  ...
  async updateAttemptStatus(attemptId: string, status: string, updatedAt: string): Promise<boolean> {
    const existing = await this.getAttempt(attemptId);
    if (!existing) return false;
    const stmt = this.db.prepare('UPDATE task_attempts SET status = ?, updated_at = ? WHERE id = ?');
    stmt.run(status, updatedAt, attemptId);
    return true;
  }

  async updateAttemptResult(attemptId: string, u: Partial<Pick<AttemptRecord, 'piPid' | 'startedAt' | 'stoppedAt' | 'worktreePath' | 'branchName' | 'promptHash' | 'workerResultJson' | 'exitReason' | 'logPath' | 'rawLogPath'>>): Promise<boolean> {
    const existing = await this.getAttempt(attemptId);
    if (!existing) return false;
    const now = new Date().toISOString();
  ...
  async createReview(input: CreateReviewInput): Promise<ReviewRecord> {
    ...
    return {
      ...
      reworkCount: 0, mergeAllowed: false, startedAt: null, finishedAt: null, createdAt: now,
    };
  }
  ...
  async updateReviewResult(reviewId: string, u: Partial<Pick<ReviewRecord, 'status' | 'reviewJson' | 'findingsJson' | 'requiredReworkJson' | 'reworkCount' | 'mergeAllowed' | 'startedAt' | 'finishedAt'>>): Promise<boolean> {
    const sets: string[] = [];
    const values: any[] = [];
    for (const [key, value] of Object.entries(u)) {
      if (value !== undefined) {
  ...
  async updateIntegrationBatch(batchId: string, u: Partial<Pick<IntegrationBatchRecord, 'status' | 'baseCommit' | 'mergeCommitHash' | 'targetMergeCommit' | 'conflictsJson' | 'finishedAt'>>): Promise<boolean> {
    const sets: string[] = [];
    const values: any[] = [];
    for (const [key, value] of Object.entries(u)) {
      if (value !== undefined) {
      ...
      switch (action.actionType) {
        case 'mark_attempt_interrupted': {
          ...
          if (existing && existing.status !== 'interrupted') {
            this.db.prepare('UPDATE task_attempts SET status = ?, exit_reason = ?, stopped_at = ?, updated_at = ? WHERE id = ?')
              .run('interrupted', `reconciled: ${action.metadata.reason || 'unknown'}`, now, now, action.targetEntityId);
            return { actionType: action.actionType, targetEntityId: action.targetEntityId, success: true };
          }
          return { actionType: action.actionType, targetEntityId: action.targetEntityId, success: true }; // idempotent
        }
        ...
        case 'release_lock': {
          ...
          if (lock && lock.status === 'locked') {
            this.db.prepare('UPDATE path_locks SET status = ?, released_at = ? WHERE id = ?')
              .run('released', now, action.targetEntityId);
            return { actionType: action.actionType, targetEntityId: action.targetEntityId, success: true };
          }
          return { actionType: action.actionType, targetEntityId: action.targetEntityId, success: true }; // idempotent
        }
        ...
        case 'mark_stage_paused': {
          const existing = this.db.prepare('SELECT status FROM stages WHERE id = ?')
            .get(action.targetEntityId) as Record<string, unknown> | undefined;
          if (existing && existing.status !== 'paused' && existing.status !== 'completed' && existing.status !== 'canceled') {
            this.db.prepare('UPDATE stages SET status = ?, updated_at = ? WHERE id = ?')
              .run('paused', now, action.targetEntityId);
            return { actionType: action.actionType, targetEntityId: action.targetEntityId, success: true };
          }
          return { actionType: action.actionType, targetEntityId: action.targetEntityId, success: true }; // idempotent
        }
        ...
        case 'mark_attempt_canceled': {
          ...
          if (existing && existing.status !== 'canceled') {
            this.db.prepare('UPDATE task_attempts SET status = ?, exit_reason = ?, stopped_at = ?, updated_at = ? WHERE id = ?')
              .run('canceled', `reconciled: ${action.metadata.reason || 'canceled_run_recovery'}`, now, now, action.targetEntityId);
            return { actionType: action.actionType, targetEntityId: action.targetEntityId, success: true };
          }
          return { actionType: action.actionType, targetEntityId: action.targetEntityId, success: true };
        }
        ...
        case 'mark_stage_canceled': {
          ...
          if (existing && existing.status !== 'canceled') {
            this.db.prepare('UPDATE stages SET status = ?, updated_at = ? WHERE id = ?')
              .run('canceled', now, action.targetEntityId);
            return { actionType: action.actionType, targetEntityId: action.targetEntityId, success: true };
          }
          return { actionType: action.actionType, targetEntityId: action.targetEntityId, success: true };
        }
        ...
        case 'update_integration_batch_completed': {
          ...
          if (batch && batch.status !== 'completed' && batch.status !== 'failed') {
            this.db.prepare('UPDATE integration_batches SET status = ?, finished_at = ? WHERE id = ?')
              .run('completed', now, action.targetEntityId);
            return { actionType: action.actionType, targetEntityId: action.targetEntityId, success: true };
          }
          return { actionType: action.actionType, targetEntityId: action.targetEntityId, success: true }; // idempotent
        }
        ...
        case 'update_approval_expired': {
          this.db.prepare('UPDATE approval_decisions SET status = ?, updated_at = ? WHERE id = ?')
            .run('expired', now, action.targetEntityId);
          return { actionType: action.actionType, targetEntityId: action.targetEntityId, success: true };
        }
        ...
        case 'update_attempt_status_by_review': {
          ...
          let newStatus = String(latestReview.status);
          if (newStatus === 'approved' || newStatus === 'rework_required') {
            this.db.prepare('UPDATE task_attempts SET status = ?, updated_at = ? WHERE id = ?')
              .run(newStatus, now, action.targetEntityId);
            return { actionType: action.actionType, targetEntityId: action.targetEntityId, success: true };
          }
