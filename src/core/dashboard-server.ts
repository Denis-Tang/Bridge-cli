  ...
  <script>
    ...
    function render(snapshot){
      ...
      document.getElementById('runs').innerHTML=runs.length?runs.map(run=>{
        ...
        const cost=run.cost?'<p class="sub">调用配额：'+esc(run.cost.committed)+' / '+esc(run.cost.limit)+' <span title="抽象配额，不是货币：本系统没有 token→金额价目表，这个数字只限制还能发起多少次最坏情况的调用">（无单位，非金额）</span>'+(b?' · reserved '+esc(b.reserved)+' · spawned '+esc(b.spawned)+' · unavailable '+esc(b.unavailable)+' · written_off '+esc(b.written_off)+' · settled '+esc(b.settled):'')+'</p>':'';
